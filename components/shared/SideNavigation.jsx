    ...
    <div className='relative max-sm:hidden flex-grow h-full bg-neutral-950 p-4 max-md:max-w-fit max-w-[200px] w-full border-r border-neutral-800'>
        {
            navElements.map((link,index)=>{
                    ...
                    return (<Link key={index} href={{pathname:link.route}} className={`transit flex items-center text-white  w-full  p-2 px-4 max-md:px-2 max-md:ml-0 block rounded-md mb-4 ${isActive ? 'bg-violet-500 hover:bg-violet-500' : 'bg-neutral-950 hover:bg-neutral-800'}`}>
                                <link.icon size={20}/>
